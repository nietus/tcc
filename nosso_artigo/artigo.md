...
O Problema de Roteamento de Veículos (\textit{Vehicle Routing Problem}, VRP) \cite{dantzig1959truck} é uma das bases da otimização logística. Suas variantes incorporam restrições operacionais como janelas de tempo, capacidade, múltiplas rotas e pontos de recarga. Entre elas, o E-VRPTW \cite{schneider2014evrptw,goeke2019evrptwdata} é a referência mais próxima deste trabalho: veículos com bateria limitada devem atender clientes dentro de janelas de tempo e podem visitar estações intermediárias.

Drones de entrega tornam esse problema mais restritivo. A autonomia é menor, a capacidade de carga é reduzida e a existência de pontos de recarga passa a determinar quais rotas são viáveis. Apesar disso, muitos trabalhos de roteamento de UAVs assumem que a infraestrutura já está disponível desde o início \cite{shi2023uav,coelho2017green,ribeiro2020uav,vichitkunakorn2024stocktaking}. Em uma implantação real, a operação tende a crescer aos poucos: novos clientes entram na área atendida e novas estações são instaladas gradualmente.

Este artigo investiga essa situação por meio do PRMD-ER, uma extensão multi-período \textit{offline} do problema E-VRPTW reinterpretada para drones. Em cada período, há um conjunto de clientes ativos e um conjunto de estações disponíveis; ambos crescem ao longo do horizonte. O objetivo é medir quanto da qualidade obtida com infraestrutura completa pode ser preservado quando as estações externas são ativadas gradualmente.

A MP-EAVNS combina construção randomizada, cinco vizinhanças de clientes, reparo energético por visitas a estações existentes e inicialização herdada entre períodos. A contribuição do trabalho é tripla: formalizar o cenário multi-período com ativação gradual de estações, avaliar o efeito dessa ativação sobre a viabilidade energética e comparar uma VNS sensível à energia com heurísticas construtivas, GRASP e uma ALNS adaptada. As janelas de tempo e a bateria são tratadas como restrições flexíveis: violações são permitidas, mas penalizadas, e os resultados separam objetivo penalizado, viabilidade energética e distância em soluções viáveis.

As Seções~\ref{sec:relacionados}, \ref{sec:metodologia}, \ref{sec:resultados} e~\ref{sec:conclusao} apresentam, respectivamente, a literatura relacionada, a formulação e o método, os resultados experimentais e as conclusões.
